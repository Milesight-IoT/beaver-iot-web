...
export class MMKVStorage implements IStorage {
    prefix: string;
    maxAge?: IStorage['maxAge'];
    mmkv: InstanceType<typeof MMKV>;

    constructor({ prefix = DEFAULT_CACHE_PREFIX, maxAge }: IStorage) {
        this.prefix = prefix;
        this.maxAge = maxAge;
        this.mmkv = new MMKV();
    }
    ...
    setItem(key: IData['key'], value: IData['value'], maxAge: IData['maxAge'] = this.maxAge) {
        ...
        this.mmkv.set(k, JSON.stringify(data));
    }

    /** 获取值 */
    getItem<T = any>(key: IData['key']): T | undefined {
        const k = `${this.prefix}${key}`;
        const dataStr = this.mmkv.getString(k) || '';
    ...
    removeItems(regex: RegExp) {
        this.mmkv.getAllKeys().forEach(key => {
