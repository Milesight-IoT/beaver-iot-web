...
const SmtpIntlKey = {
    host: 'credentials.label.smtp_addr',
    port: 'credentials.label.smtp_port',
    username: 'user.label.user_name_table_title',
    accessSecret: 'common.label.password',
    encryption: 'credentials.label.smtp_encryption',
};
...
const WhiteLabel = () => {
    ...
    const [smtpDetail, setSmtpDetail] = useState<CredentialType | undefined>();
    ...
    const smtpConfig: DescriptionsProps['data'] = useMemo(() => {
        let config = EmptySmtpHttpData;
        if (smtpDetail) {
            config = pick({ ...smtpDetail, ...(smtpDetail?.additionalData || EmptySmtpHttpData) }, [
                'host',
                'port',
                'username',
                'accessSecret',
    ...
    return (
        <div className="ms-credentials-white-label">
            <ConfigCell
                title={getIntlText('credentials.label.smtp_title')}
