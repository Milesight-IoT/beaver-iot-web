...
import { Route, SceneRendererProps, NavigationState, Scene, TabBarItemProps, Layout, Event } from './types';
import { TabBarIndicatorProps as IndicatorProps, TabBarIndicator } from './TabBarIndicator';
import { useAnimatedValue } from './useAnimatedValue';
import { MENU_ITEM_WIDTH, getMenuItemVisible } from './useMenuItem';
...
export type Props<T extends Route> = SceneRendererProps & {
	...
	onTabBarWidthChange?: (width: number, tabWidths: { [key: string]: number }) => void;
...
const getComputedTabWidth = (
	...
	if (flattenedWidth === 'auto') {
		try {
			...
			console.log("🚀 | getComputedTabWidth error:", error, index, routes, tabWidths);
			return 0;
		}
	}
...
const getTabBarWidth = <T extends Route>({
	...
	const rawTabBarWidth = routes.reduce<number>(
		(acc, _, i) =>
			acc +
			(i > 0 ? gap ?? 0 : 0) +
			getComputedTabWidth(
	...
	const menuItemVisible = getMenuItemVisible(rawTabBarWidth, layout.width);
	//如果更多菜单可见，则需要加上更多菜单的宽度
	const tabBarWidth = menuItemVisible ? rawTabBarWidth + MENU_ITEM_WIDTH : rawTabBarWidth;
...
export function TabBar<T extends Route>({
	...
	const { routes } = navigationState;
	const flattenedTabWidth = getFlattenedTabWidth(tabStyle) as any;
	...
	const { tabBarWidth, rawTabBarWidth } = getTabBarWidth({
		layout,
		navigationState,
		tabWidths,
		gap,
	...
	React.useEffect(() => {
		if(typeof onTabBarWidthChange === 'function') {
			onTabBarWidthChange(rawTabBarWidth, tabWidths);
		}
	}, [ onTabBarWidthChange, rawTabBarWidth, tabWidths ]);
			...
			const props: TabBarItemProps<T> & { key: string } = {
				...
				defaultTabWidth: !isWidthDynamic
					? getComputedTabWidth(
						...
						scrollEnabled,
						tabWidths,
							getFlattenedTabWidth(tabStyle) as any
					)
					: undefined,
				android_ripple,
			};
		...
		<Animated.View onLayout={handleLayout} style={[ styles.tabBar, style ]}>
			<Animated.View
				...
				style={[
					...
					tabBarWidth > separatorsWidth
						...
						: scrollEnabled
							? { width: tabBarWidthPercent as any }
							: null,
					indicatorContainerStyle,
				]}
			>
			...
			<View style={styles.scroll}>
				<Animated.FlatList
					...
					contentContainerStyle={contentContainerStyleMemoized as any}
